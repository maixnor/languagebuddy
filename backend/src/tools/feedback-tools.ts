...
export function getFeedbackTools(feedbackServiceInstance: FeedbackService) {
  ...
  const collectFeedbackTool: DynamicStructuredTool = new DynamicStructuredTool({
    ...
    func: async (input: FeedbackContractType) => {
      ...
      try {
        ...
        await feedbackServiceInstance.saveFeedback(feedbackEntry);
