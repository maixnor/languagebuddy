  ...
  private async getConversationHistory(phoneNumber: string): Promise<any[]> {
    ...
    try {
      ...
      const formattedMessages = messages.map((msg: any, index: number) => {
        ...
        let content = '';
        if (msg.content) { // First, check for direct content property
          content = msg.content;
        } else if (msg.kwargs?.content) {
...
`;
    ...
    try {
      ...
      const structuredLlm = (this.llm as any).withStructuredOutput(DigestAnalysisSchema);
