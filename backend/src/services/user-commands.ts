...
export async function handleUserCommand(subscriber: Subscriber, message: string) {
    ...
    if (message.startsWith('!profile')) {
        await whatsappService.sendMessage(subscriber.connections.phone, "To update your profile, please tell me your name, timezone, and any other info you'd like to update. (This feature will be improved soon!)");
        return '!profile';
    }
    ...
    if (message.startsWith('!reset')) {
        await languageBuddyAgent.clearConversation(subscriber.connections.phone);
        await whatsappService.sendMessage(subscriber.connections.phone, "Your conversation and profile have been reset to default (except your payment status).");
        return '!reset';
    }
